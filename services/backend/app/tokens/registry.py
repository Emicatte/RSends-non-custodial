...
class TokenInfo:
    ...
    coingecko_id: str
    min_amount: float
    # Assumed USD peg for stablecoins (RSends moves stablecoins only — the
    # dashboard volume figure is display-only and does not price-feed).
    # None = non-stablecoin: counted in transaction counts, contributes 0 to
    # the USD volume KPI. Set only for tokens genuinely pegged to 1 USD.
    peg_usd: Optional[float] = None
...
_register(TokenInfo("USDC",  "USD Coin",         6, "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913",      8453, False, "usd-coin", 0.01, peg_usd=1.0))
_register(TokenInfo("USDT",  "Tether USD",       6, "0xfde4C96c8593536E31F229EA8f37b2ADa2699bb2",      8453, False, "tether",   0.01, peg_usd=1.0))
_register(TokenInfo("DAI",   "Dai Stablecoin",  18, "0x50c5725949A6F0c72E6C4a641F24049A917DB0Cb",      8453, False, "dai",      0.01, peg_usd=1.0))
...
_register(TokenInfo("USDC", "USDC (Sepolia)",     6, "0x036CbD53842c5426634e7929541eC2318f3dCF7e",      84532, False, "usd-coin", 0.01, peg_usd=1.0))

# ═══ ETHEREUM MAINNET (1) ═══
_register(TokenInfo("ETH",  "Ether",            18, None,                                                   1, True,  "ethereum", 0.001))
_register(TokenInfo("USDC", "USD Coin",           6, "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48",          1, False, "usd-coin", 0.01, peg_usd=1.0))
_register(TokenInfo("USDT", "Tether USD",         6, "0xdAC17F958D2ee523a2206206994597C13D831ec7",          1, False, "tether",   0.01, peg_usd=1.0))

# ═══ ARBITRUM ONE (42161) ═══
_register(TokenInfo("ETH",  "Ether",            18, None,                                               42161, True,  "ethereum", 0.0001))
_register(TokenInfo("USDC", "USD Coin",           6, "0xaf88d065e77c8cC2239327C5EDb3A432268e5831",      42161, False, "usd-coin", 0.01, peg_usd=1.0))
_register(TokenInfo("USDT", "Tether USD",         6, "0xFd086bC7CD5C481DCC9C85ebE478A1C0b69FCbb9",      42161, False, "tether",   0.01, peg_usd=1.0))
