import type { NextAuthOptions } from 'next-auth'
import CredentialsProvider from 'next-auth/providers/credentials'
import { requireEnv } from '@/lib/env'

/**
 * NextAuth v4 options shared between the route handler and any
 * server-side caller that needs `getServerSession(authOptions)`.
 *
 * Credentials (email+password) is the only provider — social login
 * (Google/GitHub) was removed from the product. The client POSTs
 * email+password to /api/rp-auth/api/v1/auth/login to set the httpOnly
 * cookies and get an access_token, then calls
 * signIn('credentials', { access_token, ... }). This provider verifies the
 * token server-to-server against /auth/me and seeds the JWT with
 * access_token so downstream hooks that gate on
 * useSession().status === 'authenticated' keep working.
 */

export const authOptions: NextAuthOptions = {
  providers: [
    CredentialsProvider({
      id: 'credentials',
      name: 'Email and password',
      credentials: {
        access_token: { type: 'text' },
        user_id: { type: 'text' },
        email: { type: 'text' },
      },
      async authorize(credentials) {
        if (!credentials?.access_token) return null
        // Lazy: resolve the backend URL here (not at module load) so a missing
        // RPAGOS_BACKEND_URL can't throw at import and break the auth subsystem.
        const BACKEND_URL = requireEnv('RPAGOS_BACKEND_URL')
        try {
          const res = await fetch(`${BACKEND_URL}/api/v1/auth/me`, {
            headers: { Authorization: `Bearer ${credentials.access_token}` },
            cache: 'no-store',
          })
          if (!res.ok) return null
          const user = (await res.json()) as {
            id?: string
            email?: string
            display_name?: string | null
            email_verified?: boolean
          }
          return {
            id: user.id ?? credentials.user_id ?? 'unknown',
            email: user.email ?? credentials.email ?? '',
            name: user.display_name ?? user.email ?? credentials.email ?? null,
            access_token: credentials.access_token,
            email_verified: user.email_verified ?? false,
          } as unknown as {
            id: string
            email: string
            name: string | null
            access_token: string
            email_verified: boolean
          }
        } catch {
          return null
        }
      },
    }),
  ],
  session: { strategy: 'jwt', maxAge: 30 * 60 },
  callbacks: {
    async jwt({ token, account, user, trigger, session }) {
      if (
        account?.provider === 'credentials' &&
        user &&
        (user as unknown as Record<string, unknown>).access_token
      ) {
        ;(token as Record<string, unknown>).access_token = (user as unknown as Record<string, unknown>).access_token
        ;(token as Record<string, unknown>).email_verified = (user as unknown as Record<string, unknown>).email_verified
      }
      if (trigger === 'update' && session && (session as Record<string, unknown>).access_token) {
        ;(token as Record<string, unknown>).access_token = (session as Record<string, unknown>).access_token
      }
      return token
    },
    async session({ session, token }) {
      ;(session as unknown as Record<string, unknown>).access_token = (token as Record<string, unknown>).access_token
      ;(session as unknown as Record<string, unknown>).email_verified = (token as Record<string, unknown>).email_verified
      return session
    },
  },
}
