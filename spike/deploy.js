...
(async () => {
  ...
  // Retire the previous deployment before overwriting the record. Losing it
  // silently is how an address stays "current" in a file after it has been
  // superseded on chain.
  const depPath = path.join(__dirname, 'deployed.json');
  const retPath = path.join(__dirname, 'retired.json');
  if (fs.existsSync(depPath)) {
    const prev = JSON.parse(fs.readFileSync(depPath, 'utf8'));
    if (prev.address && prev.address !== addr) {
      const retired = fs.existsSync(retPath) ? JSON.parse(fs.readFileSync(retPath, 'utf8')) : [];
      if (!retired.some((r) => r.address === prev.address)) {
        retired.push({ ...prev, retiredOn: new Date().toISOString().slice(0, 10), reason: 'superseded by ' + addr });
        fs.writeFileSync(retPath, JSON.stringify(retired, null, 2) + '\n');
        console.log('retired previous deployment:', prev.address);
      }
    }
  }
